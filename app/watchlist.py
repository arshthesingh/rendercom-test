...
def move_movie_up():
    ...
    user = get_current_user()
    current_movie = Watchlist.query.filter_by(user_id=user.id, movie_title=movie_title).first()

    if not current_movie:
        return jsonify({"error": f'"{movie_title}" is not in your watchlist'}), 404

    movie_above = Watchlist.query.filter_by(user_id=user.id, priority=current_movie.priority - 1).first()

    if movie_above:
        current_movie.priority, movie_above.priority = movie_above.priority, current_movie.priority
        db.session.commit()
...
def move_movie_down():
    ...
    movie_below = Watchlist.query.filter_by(user_id=user.id, priority=current_movie.priority + 1).first()

    if movie_below:
        current_movie.priority, movie_below.priority = movie_below.priority, current_movie.priority
        db.session.commit()
